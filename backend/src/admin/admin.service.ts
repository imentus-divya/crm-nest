...
export class AdminService {
  ...
  ManageUser = async () => {
    ...
    console.log("🚀 ~ file: admin.service.ts:90 ~ AdminService ~ users:", users)
    // return(users);
  ...
  AddUser = async () => {
    ...
    console.log("🚀 ~ file: admin.service.ts:107 ~ AdminService ~ roles:", roles)
    return roles;

  }
  ManageRoles = async () => {
    console.log("🚀 ~ file: admin.service.ts:105 ~ AdminService ~ ManageRoles:")
    const roles = await this.rolesRepository.find();

    console.log("🚀 ~ file: admin.service.ts:90 ~ AdminService ~ users:", roles)
    // return(users);
  ...
  async SaveUser(req: Request): Promise<any> 
  {
    ...
    // Insert New User
     ...
     {
      const user_role=await this.rolesRepository.findOne({select:['id'],where:{name:'User'}})
      ...
      user.country_code=1;
      console.log("🚀 ~ file: admin.service.ts:145 ~ AdminService ~ SaveUser ~ user:", user.role_id,"---------type-----",typeof(user.role_id))
      try{
        const saveUser=await this.userRepository.save(user)
        const newUser_id=saveUser.id as number;
        console.log("🚀 ~ file: admin.service.ts:147 ~ AdminService ~ SaveUser ~ newUser_id:", newUser_id,"type is : ",typeof(newUser_id))
        return newUser_id;
