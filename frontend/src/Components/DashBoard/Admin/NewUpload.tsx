...
const NewUpload = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!selectedCounty || !selectedfileType) {
      toast.current?.show({
        severity: "error",
        summary: "Select All the Options",
      });
    ...
    if (e.target.files && e.target.files.length > 0) {
      ...
      reader.onload = (event) => {
        if (event.target && typeof event.target.result === "string") {
          ...
          console.log(
            "🚀 ~ file: NewUpload.tsx:57 ~ handleFileChange ~ parsedData:",
            parsedData
          );

          console.log(
            "🚀 ~ file: NewUpload.tsx:57 ~ handleFileChange ~ handleData:"
          );
  ...
  const handleData = (csvD: any) => {
    console.log(
      "🚀 ~ file: NewUpload.tsx:95 ~ handleData ~ csvData: length",
      csvD.length
    );
    const headers = csvD[0];
    console.log("🚀 ~ file: NewUpload.tsx:95 ~ handleData ~ headers:", headers);

    const _rowDataObjects = csvD.slice(1);
    ...
    console.log(
      "🚀 ~ file: NewUpload.tsx:93 ~ dataAsObjects ~ dataAsObjects:",
      dataAsObjects
    );
        ...
        <div className="main-content-admin">
          <div className="main-admin">
            ...
            <div className="new-upload-filters">
              ...
              <div className="card flex justify-content-center new-upload-filters-btn">
                <Button
                  label="Sample File"
                  onClick={() => {
                    hadleSampleFileDownload(sample_foreclosure_url);
                  }}
                />
              </div>
            </div>
            ...
            <div className="new-upload-data">
              <div className="new-upload-data-top"></div>
